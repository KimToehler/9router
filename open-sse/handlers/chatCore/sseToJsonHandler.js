...
export async function handleForcedSSEToJson({ providerResponse, sourceFormat, targetFormat, provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, customToolNames, trackDone, appendLog, reqTag, log }) {
  ...
  if (isCodexResponsesApi) {
    try {
      ...
      if (sourceFormat === FORMATS.OPENAI_RESPONSES) {
        return { success: true, response: new Response(JSON.stringify(jsonResponse), { headers: JSON_HEADERS_CORS }) };
      }
      ...
      return { success: true, response: new Response(JSON.stringify(finalResp), { headers: JSON_HEADERS_CORS }) };
    } catch (err) {
  ...
  try {
    ...
    return { success: true, response: new Response(JSON.stringify(finalBody), { headers: JSON_HEADERS_CORS }) };
  } catch (err) {
